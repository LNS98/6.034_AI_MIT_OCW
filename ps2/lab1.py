...
brother = IF( AND( 'male (?x)', 'parent (?z) (?x)', 'parent (?z) (?y)', NOT('same identity (?x) (?y)')),
          THEN('brother (?x) (?y)'))

sister = IF( AND( 'female (?x)', 'parent (?z) (?x)', 'parent (?z) (?y)', NOT('same identity (?x) (?y)')),
          THEN('sister (?x) (?y)'))
...
cousin = IF( AND('grandparent (?z) (?x)', 'grandparent (?z) (?y)',
             NOT('sibling (?x) (?y)'),
             NOT('same identity (?x) (?y)')),
             THEN('cousin (?x) (?y)'))

grandparent = IF( AND('parent (?z) (?x)', 'parent (?y) (?z)'),
             THEN('grandparent (?y) (?x)'))

grandchild = IF( AND('parent (?z) (?x)', 'parent (?y) (?z)'),
             THEN('grandchild (?x) (?y)'))

# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [same_identity, brother, sister, sibling, father, son,
                daughter, grandparent, grandchild, cousin]                    # fill me in
...
WHAT_I_FOUND_BORING = 'nothing in particular'
